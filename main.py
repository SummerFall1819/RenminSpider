...
from spiderlog import init_log,box_alert,wx_alert
...
class RUCSpider(object):
    ...
    def CheckSettings(self):
        ...
        if notifier == "box_alert":
            self.notifier = lambda *args, **kwargs: box_alert(*args, **kwargs, icon_path=self.icon_path)
        elif notifier == "wx_alert":
            self.notifier = lambda *args, **kwargs: wx_alert(*args, **kwargs, uid=info["wxpusher_uid"], appToken=info["wxpusher_appToken"])
        else:
            self.notifier = lambda **kwargs: None
    ...
    def PullLecture(self, maxlen:int = 30,Condition:list = None, Query:str = "", filter = lambda x: True):
        ...
        self.logger.info(f"Check complete: filtered {len(new_id)} out of {len(lec)}.")
        if not hasattr(self, "_start_listen_notification"):
            self._start_listen_notification = True
            self.notifier(title="Start Listen", msg="Check complete: filtered {} out of {}.".format(len(new_id),len(lec)), url="https://v.ruc.edu.cn/campus#/activity/partakelist/3")
    ...
    def register(self,aid:list):
        ...
        for idx in success_lec:
            self.notifier(title="Notice", msg="Lecture {} successfully registered.".format(str(success_lec)), url=f"https://v.ruc.edu.cn/campus#/activity/partakedetail/{idx}/description")
