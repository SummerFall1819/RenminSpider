...
default_formats = {
    # 终端输出格式
    'color_format': '%(log_color)s[%(asctime)s]|[line:%(lineno)d]-%(levelname)s:%(message)s',
    # 日志输出格式
...
class HandleLog:
    ...
    def __set_color_formatter(console_handle, color_config):
        ...
        :return:
        """
        formatter = colorlog.ColoredFormatter(default_formats["color_format"],datefmt="%Y-%m-%d %H:%M:%S", log_colors=color_config)
